...
Timer_init:
		...
		ldi temp, (1<<CS10)			
		sts TCCR1B, temp			; Configuro el timer con fckl
		
		ldi temp, 0x00
		sts TCNT1H, temp			; El timer comienza de 0000
		sts TCNT1L, temp
		
		ldi temp, 0xFF				
		sts	OCR1AH, temp			; Cuando llega a FFFF que salte la interrupcion
		ldi temp, 0xFF
		sts	OCR1AL,	temp
		
		ldi temp, (1<<OCIE1A)		; Hablito la interrupcion
		sts TIMSK1, temp
...
Handler_Int_URXC0:
		...
		lds temp2, UDR0  ; Leo lo que recibi
		cpi temp2, 1		; Lo comparo con un 1
		breq LED_ON		; Si es un 1 voy a prender el LED
		cpi temp2, 0		; Lo comparo con un 0
